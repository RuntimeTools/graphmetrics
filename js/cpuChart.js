...
var cpuResize = cpuSVG.append("image")
    ...
    .on("click", function(){
        ...
        if(cpuChartIsFullScreen) {
            d3.select(".cpuChart .maximize").attr("xlink:href","graphmetrics/images/minimize_24_grey.png")
            // Redraw this chart only
            resizeCPUChart();
        } else {
            canvasWidth = $("#cpuDiv1").width() - 8; // -8 for margins and borders
            graphWidth = canvasWidth - margin.left - margin.right;
            d3.select(".cpuChart .maximize").attr("xlink:href","graphmetrics/images/maximize_24_grey.png")
            canvasHeight = 250;
    ...
    .on("mouseover", function() {
        if(cpuChartIsFullScreen) {
            d3.select(".cpuChart .maximize").attr("xlink:href","graphmetrics/images/minimize_24.png")
        } else {
            d3.select(".cpuChart .maximize").attr("xlink:href","graphmetrics/images/maximize_24.png")
        }
    })
